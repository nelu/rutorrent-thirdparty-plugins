...
AUTH_USER,a:,auth:,USER:PASSWORD,Use an account"
...
rapidshare_upload() {
    ...
    if test -n "$AUTH_USER"; then
	 rapidshare_upload_user "$@"
    else
	log_error "Authorization is required!"
	return 1;
    fi
}
...
rapidshare_upload_user() {
	...
	local LOGIN=$(echo "$AUTH_USER" | cut -d : -f 1)
	local PASS=$(echo "$AUTH_USER" | cut -d : -f 2)
    local FILE="$2"
